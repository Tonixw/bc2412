  ...
  public static void main(String[] args) {
    ...
    List<Integer> newNumbers =
        numbers.stream().filter(e -> e % 2 == 0).map(e -> e * e)
            .sorted((e1, e2) -> e1 > e2 ? -1 : 1).collect(Collectors.toList());
    ...
    List<String> newNames = names.stream().filter(e -> e.indexOf('A') == 0)
    ...
    // Output: Max: 30
    // Output: Min: 5
    int max = numbers2.stream().max((n1, n2) -> n1.compareTo(n2)).orElse(-1);
    System.out.println("Max: " + max);
    int min = numbers2.stream().min((n1, n2) -> n1.compareTo(n2)).orElse(-1); // orElse() to open the "box"
    ...
    List<Integer> newWords =
        words.stream().map(name -> name.length()).collect(Collectors.toList());
    ...
    // student.get(0).setScore(80); // OK
    Map<String, Integer> result7 = students.stream()
