...
public class DemoBigDecimal {
  public static void main(String[] args) {
    ...
    System.out.println(0.3-0.1); // 0.199999999999998
    ...
    double circleaArea = BigDecimal.valueOf(Math.pow(2.0, radius))
    .multiply(BigDecimal.valueOf(Math.PI))
    .doubleValue();
    System.out.println(circleArea);
    ...
    max = Math.max(x, max); // if (x>mam){max=x;}
    max = Math.max(y, max);
  ...
  // 3.46
  System.out.println(Math.round(u * 100) / 100); // 3.46
